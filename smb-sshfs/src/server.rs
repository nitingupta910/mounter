...
impl SmbSession {
    ...
    fn handle_negotiate(&mut self, hdr: &Smb2Header, body: &[u8], out: &mut Vec<u8>) {
        ...
        resp.extend_from_slice(&(256 * 1024u32).to_le_bytes()); // MaxTransactSize
        resp.extend_from_slice(&(256 * 1024u32).to_le_bytes()); // MaxReadSize
        resp.extend_from_slice(&(256 * 1024u32).to_le_bytes()); // MaxWriteSize
        ...
        resp.extend_from_slice(&spnego_init);
...
fn build_spnego_init() -> Vec<u8> {
    // NTLMSSP OID: 1.2.840.113554.1.2.2.10  (simplified)
    // Build a minimal SPNEGO NegTokenInit advertising NTLMSSP
    let ntlmssp_oid = [
        0x06, 0x0a, 0x2b, 0x06, 0x01, 0x04, 0x01, 0x82, 0x37, 0x02, 0x02, 0x0a,
    ];
    let mech_list = asn1_sequence(&ntlmssp_oid);
    let mech_types = asn1_context(0, &mech_list);
    let neg_token_init = asn1_sequence(&mech_types);
